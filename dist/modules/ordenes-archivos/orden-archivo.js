...
const orden_archivo_schemas_1 = require("./orden-archivo.schemas");
const orden_archivo_service_1 = require("./orden-archivo.service");
...
async function listArchivos(req, res) {
    try {
        ...
        const archivos = await (0, orden_archivo_service_1.getArchivosService)(ordenServicioId, empresaId);
        return res.json({
            success: true,
...
async function createArchivoHandler(req, res) {
    try {
        ...
        const { url, storageKey } = await (0, uploadToSpaces_1.uploadToSpaces)(file, "ordenes", ordenServicioId);
        const parsed = orden_archivo_schemas_1.createArchivoSchema.parse({
            nombre: req.body.nombre,
            categoria: req.body.categoria,
            mimeType: file.mimetype,
            storageKey,
        });
        const archivo = await (0, orden_archivo_service_1.createArchivoService)(ordenServicioId, empresaId, usuarioId, {
            ...parsed,
...
async function updateArchivoEstadoHandler(req, res) {
    try {
        ...
        const parsed = orden_archivo_schemas_1.updateArchivoEstadoSchema.parse(req.body);
        const archivo = await (0, orden_archivo_service_1.updateArchivoEstadoService)(id, ordenServicioId, empresaId, parsed.estado);
        return res.json({
            success: true,
